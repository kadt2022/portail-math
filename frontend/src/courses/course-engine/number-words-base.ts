// Grammaire commune de l'écriture des nombres de 0 à 999 en toutes lettres,
// en français (RDC : septante/nonante ne sont pas utilisés) et en anglais.
// Un cours au-delà de 999 (ex. la 4e primaire, jusqu'à 99 999) compose ses
// propres groupes de milliers par-dessus ces fonctions plutôt que de
// redéfinir ces règles de grammaire de base.
...
export function frTensAndUnits(n: number): string {
  if (n < 20) {
    return FR_UNITS[n];
  }
  if (n < 70 || (n >= 80 && n < 90)) {
...
export function frBelowThousand(n: number): string {
  if (n === 0) {
    return FR_UNITS[0];
  }
...
export function enTensAndUnits(n: number): string {
  if (n < 20) {
    return EN_UNITS[n];
  }
  const tensDigit = Math.floor(n / 10);
...
export function enBelowThousand(n: number): string {
  if (n === 0) {
    return EN_UNITS[0];
  }
  const hundreds = Math.floor(n / 100);
