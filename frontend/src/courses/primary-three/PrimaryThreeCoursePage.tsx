...
export function PrimaryThreeCoursePage() {
  ...
  const { progress } = useCourseProgress(PRIMARY_THREE_COURSE.id);
  const summary = getCourseProgress(progress, PRIMARY_THREE_COURSE);
  const primaryAction = getCoursePrimaryAction(progress, PRIMARY_THREE_COURSE);

  return (
    <div className={baseStyles.page}>
      <section className={baseStyles.courseHero} aria-labelledby="primary-three-title">
        <div className={baseStyles.heroCopy}>
          <p className={baseStyles.eyebrow}>{t("course.eyebrow")}</p>
          <h1 id="primary-three-title">{t("course.title")}</h1>
          <p className={baseStyles.lead}>{t("course.lead")}</p>
          <p className={styles.publishedNote}>{t("course.published", { count: 5 })}</p>
      ...
      <section className={baseStyles.modulesSection} aria-labelledby="primary-three-modules">
        <div className={baseStyles.sectionHeading}>
          <div>
            <p className={baseStyles.eyebrow}>{t("modules.eyebrow")}</p>
            <h2 id="primary-three-modules">{t("modules.title")}</h2>
          </div>
          <p>{t("modules.orderHint")}</p>
        </div>

        <div className={baseStyles.moduleGrid}>
          {PRIMARY_THREE_MODULES.map((module) => {
