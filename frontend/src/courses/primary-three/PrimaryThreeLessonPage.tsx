...
import {
  getPrimaryThreeItem,
  getPrimaryThreeModule,
  lessonPath,
  modulePath,
  PRIMARY_THREE_BASE_PATH,
  PRIMARY_THREE_COURSE,
  PRIMARY_THREE_MODULES,
} from "./course-catalogue";
import { PRIMARY_THREE_EVALUATION_CONTENT, PRIMARY_THREE_LESSON_CONTENT } from "./content";
import { InteractiveExercise } from "./exercises/InteractiveExercise";
import styles from "./PrimaryThreeLesson.module.css";

export function PrimaryThreeLessonPage() {
  const { moduleId, lessonId } = useParams();
  const { t } = useTranslation("primaryThree");
  const { progress, startItem, completeStep } = useCourseProgress(PRIMARY_THREE_COURSE.id);
  const module = getPrimaryThreeModule(moduleId);
  const item = module ? getPrimaryThreeItem(module, lessonId) : undefined;
  const [viewByItem, setViewByItem] = useState<Record<string, string | "done">>({});

  const lessonContent = item?.kind === "lesson" ? PRIMARY_THREE_LESSON_CONTENT[item.id] : undefined;
  const evaluationContent = item?.kind === "evaluation" ? PRIMARY_THREE_EVALUATION_CONTENT[item.id] : undefined;
  ...
  if (!module || !item || !isPublished(item) || !hasContent || !view) {
    return (
      <div className={baseStyles.missingPage}>
        ...
        <Link className={baseStyles.secondaryAction} to={PRIMARY_THREE_BASE_PATH}>
  ...
  if (view === "done") {
    ...
    const nextModule = PRIMARY_THREE_MODULES[module.number];
  ...
  function renderStep() {
    ...
    switch (step.kind) {
      ...
      case "discover":
        return <ExplanationBlock textKey={lessonContent.discoverKey} completed={stepCompleted} onValidated={onInfoConfirmed} />;
      case "manipulate":
    ...
    <div className={baseStyles.lessonPage}>
      <header className={baseStyles.lessonHeader}>
        <nav className={baseStyles.breadcrumbs} aria-label={t("breadcrumbs.label")}>
          <Link to={PRIMARY_THREE_BASE_PATH}>{t("breadcrumbs.course")}</Link>
